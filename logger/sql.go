...
func ExplainSQL(sql string, numericPlaceholder *regexp.Regexp, escaperPrefix, escaper string, avars ...interface{}) string {
	...
	convertParams = func(v interface{}, idx int) {
		...
		case fmt.Stringer:
			reflectValue := reflect.ValueOf(v)
			if v != nil && reflectValue.IsValid() && ((reflectValue.Kind() == reflect.Ptr && !reflectValue.IsNil()) || reflectValue.Kind() != reflect.Ptr) {
				vars[idx] = escaper + strings.Replace(fmt.Sprintf("%v", v), escaper, escaperPrefix+escaper, -1) + escaper
			} else {
				vars[idx] = nullStr
			}
		case []byte:
			if isPrintable(v) {
				vars[idx] = escaper + strings.Replace(string(v), escaper, escaperPrefix+escaper, -1) + escaper
			} else {
		...
		case string:
			vars[idx] = escaper + strings.Replace(v, escaper, escaperPrefix+escaper, -1) + escaper
		default:
			...
			if v == nil || !rv.IsValid() || rv.Kind() == reflect.Ptr && rv.IsNil() {
				...
				vars[idx] = escaper + strings.Replace(fmt.Sprint(v), escaper, escaperPrefix+escaper, -1) + escaper
			}
		}
	}
